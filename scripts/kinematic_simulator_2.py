...
class kinematic_simulator:
    ...
    def update_position(self):
        ...
        # self.pos_x += self.velocity_x * (1.0 / self.frequency)
        # self.pos_y += self.velocity_y * (1.0 / self.frequency)
        # self.pos_z += self.velocity_z * (1.0 / self.frequency)

        # Update yaw based on angular velocity
        # self.yaw += self.angular_velocity_yaw * (1.0 / self.frequency)
...
def main(): 
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            sim.br.sendTransform(
            (sim.pos_x, sim.pos_y, sim.pos_z),
            quaternion_from_euler(0, 0, sim.yaw),
            rospy.Time.now(),
            'base_link',   # child frame
            'NED'        # parent frame, typically 'world' or 'map'
        )
            ...
            sim.publish_pose()
            rospy.loginfo_throttle(5, f"x,y,z,yaw: {sim.pos_x}, {sim.pos_y}, {sim.pos_z}, {sim.yaw}")
            rospy.sleep(0.1)
