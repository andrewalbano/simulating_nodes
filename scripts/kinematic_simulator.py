...
from geometry_msgs.msg import TwistStamped, PoseWithCovarianceStamped, Twist, PoseStamped, TwistWithCovarianceStamped
...
class kinematic_simulator:
    def __init__(self):
        ...
        self.max_linear_deceleration = 0.05

        # deg/s
        self.max_angular_acceleration = 8*np.pi/180
        self.max_angular_deceleration = 15*np.pi/180
        ...
        # subscribe to twist 
        self.sub1 = rospy.Subscriber('velocity_setpoint', TwistStamped, self.velocity_callback)
        ...
        self.pub2 = rospy.Publisher('/twist', TwistWithCovarianceStamped, queue_size=1)
...
# # 
    ...
    def velocity_callback(self,msg:TwistStamped):
        ...
        if self.add_disturbance:
            ...
            if r>6 and r<8:
                self.velocity.linear.x += random.randint(0,1)/30
            elif r > 8:
                self.velocity.linear.x -= random.randint(0,1)/30

            r = random.randint(0,10)
            if r>6 and r<8:
                self.velocity.linear.y += random.randint(0,1)/30
            elif r > 8:
                self.velocity.linear.y -= random.randint(0,1)/30
